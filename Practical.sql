...
create login Zain with Password ='Zain'
...
SELECT P.ProductName, OD.Quantity, OD.Quantity * P.UnitPrice AS TotalPrice FROM Orders O JOIN OrderDetails OD ON O.OrderID = OD.OrderID JOIN Products P ON OD.ProductID = P.ProductID WHERE O.TotalAmount > (SELECT AVG(TotalAmount) FROM Orders);
...
SELECT OS.OrderID, OS.OrderDate, OS.CustomerID, OS.TotalAmount, C.FirstName, C.LastName FROM ORDERS OS JOIN Customers C ON OS.CustomerID = C.CustomerID;